    ...
    <div class="grid h-screen min-h-screen w-full overflow-hidden lg:grid-cols-[280px_1fr]">
          ...
          <div class="flex-1 overflow-auto py-2">
            <nav class="grid items-start px-4 text-sm font-medium">
              ...
              <router-link to="/mannage" v-show="identity != 0">
                <a class="flex items-center gap-3 rounded-lg px-3 py-2 text-blue-600 dark:text-gray-50">
                  <svg
                      xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                      fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                      stroke-linejoin="round" class="h-4 w-4">
      ...
      <div class="flex flex-col">
        ...
        <main class="flex flex-col p-6">
          <h1 class="text-2xl font-semibold mb-4">所有物流</h1>
                  ...
                  <td class="p-4 align-middle [&amp;:has([role=checkbox])]:pr-0 hidden md:table-cell">
                    ...
                    <th>
                      <button class="btn btn-error btn-xs"
                              v-if="item.Status == 'await' && identity == 1"
                              @click="acceptPackage(item.PackageId)">接收物流
                      </button>
                      <button class="btn btn-ghost btn-xs" @click="traceShipment(item.PackageId)"
                              v-if="item.Status != 'await'"> 物流追踪
                      </button>
                      <button class="btn btn-ghost btn-xs"
                              v-if="item.Status != 'await' && identity == 1"
                              @click="updatePackageInfo(item.PackageId)">
                        更新物流状态
                      </button>
                    </th>
                  </td>
...
export default {
  ...
  created() {
    ...
    this.initAMap();
    this.$axios.get("/admin/getAllPackage", {
      params: {
        page: this.page,
        size: 10
      }
  ...
  methods: {
    ...
    // 接收物流
    acceptPackage(packageId) {
      this.$axios.post("/admin/receivePackage", {
      ...
      window.AMap.plugin('AMap.Geolocation', () => {
        ...
        geolocation.getCurrentPosition(async (status, result) => {
          if (status === 'complete') {
            // 定位成功，获取当前地址
            console.log(result)
            const {lat: latitude, lng: longitude} = result.position;
            var currentAddress = await this.getAddress(longitude, latitude);
            console.log(currentAddress)

            // 发送更新请求
            this.sendUpdateRequest(package_id, currentAddress);
          } else {
            // 定位失败
    ...
    initAMap() {
      AMapLoader.load({
        key: "25125b66d60b8e29dfdde781c72bf6cd", // 使用环境变量或其他方式获取密钥
        version: '2.0',
        plugins: [
    ...
    getAddress(lng, lat) {
      ...
      return new Promise((resolve, reject) => {
        this.$axios.get('https://restapi.amap.com/v3/geocode/regeo?key=29b5f74e7e7f47e8ef034e6cc0e8db5f&location=' + lng + ',' + lat, {
          headers: {
            'Authorization': undefined  // 设置为 null 或 undefined 可以删除该头部
          }
        }).then(res => {
          if (res.data.status === '1') {
            console.log(res.data.regeocode.formatted_address);
            resolve(res.data.regeocode.formatted_address); // 使用resolve来返回数据
          } else {
            reject('Failed to fetch address'); // 使用reject来处理错误情况
          }
        }).catch(error => {
          reject(error); // 处理请求错误
        });
    ...
    sendUpdateRequest(package_id, currentAddress) {
      // 发送更新请求
      this.$axios.post("/admin/updatePackageStatus", {
        email: localStorage.getItem("email"),
        package_id: package_id,
        currentAddress: currentAddress // 将当前地址添加到请求数据中
      }).then((rep) => {
        console.log(rep);
        if (rep.data.code == 200) {
          this.$message.success(rep.data.message);
